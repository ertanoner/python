...
def kayit_ara():
    ...
    okunan = dosya.read()
    cevirilen = ast.literal_eval(okunan)
    aranan = input("Aranan isim, soyad veya numarayı giriniz?")
    kayitVarMi = 1
    for a in cevirilen:
        if a["isim"] == aranan or a["soyad"]==aranan or a["numara"]==aranan  : 
            print(a)  
            kayitVarMi = 1 
            break
        else :  kayitVarMi = 0
        if kayitVarMi == 1 : break
    if kayitVarMi == 0 : print("Bu isimde bir kayit yok.") 
